...
1. Kubernetes is not opinionated, it lets you choose your own CNI solution. Until a CNI plugin is installed the cluster will be inoperable. List the running Pods in all namespaces (`-A`):
   ...
   Some pods are already running, others are pending. The pods that are running use the host network: they share their network stack with the node they are running on. This is why their IP addresses are in the `172.30.10.0/24` range, which is the range used for the nodes.

   The other pods are waiting for an IP address before they can start, so they are in a pending state. They will not start up before a network plugin is installed.

2. Install [Cilium](https://cilium.io/) as our CNI plugin.

   Installing a CNI plugin to a Kubernetes node involves essentially two tasks:

   - adding a configuration file to the `/etc/cni/net.d/` directory
   - adding a corresponding binary (called a type) to the `/opt/cni/bin/` directory

   This configuration will be used by the CRI implementation when creating and deleting pods in order to configure the networking interfaces.
